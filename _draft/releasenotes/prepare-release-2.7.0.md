# The Burning Crusade

## General
### Game Updates
The addon introduces a new flavor: The Burning Crusade.

Here is the full list of supported Spell Alerts so far
- Druid's Nature's Grace
- Hunter's Mongoose Bite
- Mage's Clearcasting
- Mage's Freeze debuff
- Priest's Surge of Light
- Rogue's Riposte
- Shaman's Elemental Focus
- Warlock's Backlash
- Warlock's Eye of Kilrogg
- Warlock's Nightfall
- Healing Trance / Eye of Gruul, for all healing classes

And here is the list of Glowing Buttons
- Hunter's Counterattack
- Hunter's Mongoose Bite
- Mage's Ice Lance, when the target is Frozen
- Priest's Shadowform, when not assuming the shadowform
- Priest's Smite, during Surge of Light
- Paladin's Exorcism
- Paladin's Hammer of Wrath
- Paladin's Holy Shock
- Rogue's Riposte
- Warlock's Incinerate, during Backlash
- Warlock's Shadow Bolt, during Backlash
- Warlock's Shadow Bolt, during Nightfall
- Warrior's Execute
- Warrior's Overpower
- Warrior's Revenge
- Warrior's Victory Rush

Make sure to report issues and suggest ideas. Thank you :)
### Glowing Buttons
Glowing Buttons are now functional.
### Druid
In the options panel, the Spell Alert option for Healing Trance now always display the item text 'Eye of Gruul' next to it.
### Paladin
In the options panel, the Spell Alert option for Healing Trance now always display the item text 'Eye of Gruul' next to it.
### Priest
In the options panel, the Spell Alert option for Healing Trance now always display the item text 'Eye of Gruul' next to it.
### Shaman
In the options panel, the Spell Alert option for Healing Trance now always display the item text 'Eye of Gruul' next to it.
## Known Limitations
Sliders in the options panel have no background ruler.
## Miscellaneous
This release optimizes code size, which have a few positive side effects
- The addon loads slightly faster
- The addon uses slightly less memory
- Packages are slightly smaller to download
- Gains are about 5-15% depending on game flavor and player class


# Wrath of the Lich King

## Bug Fixes
### Druid
In the options panel, the Spell Alert option for Healing Trance now always display the item text 'Soul Preserver' next to it.
### Paladin
In the options panel, the Spell Alert option for Healing Trance now always display the item text 'Soul Preserver' next to it.
### Priest
In the options panel, the Spell Alert option for Healing Trance now always display the item text 'Soul Preserver' next to it.
### Shaman
In the options panel, the Spell Alert option for Healing Trance now always display the item text 'Soul Preserver' next to it.
## Miscellaneous
This release optimizes code size, which have a few positive side effects
- The addon loads slightly faster
- The addon uses slightly less memory
- Packages are slightly smaller to download
- Gains are about 5-15% depending on game flavor and player class


# Vanilla / Cataclysm

## Miscellaneous
This release optimizes code size, which have a few positive side effects
- The addon loads slightly faster
- The addon uses slightly less memory
- Packages are slightly smaller to download
- Gains are about 5-15% depending on game flavor and player class


# Mists of Pandaria

## Classes
### Death Knight
New Glowing Buttons
- Blood Tap, with 5+ charges of Blood Charge
- Soul Reaper, when the target has low hp

There are three Soul Reaper options, one for each spec.

By default, Soul Reaper's execute threshold is 35% of the target's health. This threshold increases to 45% health when the Death Knight equips 4+ pieces of the tier 15 set bonus.
## Miscellaneous
This release optimizes code size, which have a few positive side effects
- The addon loads slightly faster
- The addon uses slightly less memory
- Packages are slightly smaller to download
- Gains are about 5-15% depending on game flavor and player class

## Contributors
Shout-out to our amazing contributors
- @Amanthuul my man, for adding Death Knights effects

Thanks!


# GitHub

## General
### Game Updates
The addon introduces a new flavor: The Burning Crusade.

Here is the full list of supported Spell Alerts so far
- Druid's Nature's Grace
- Hunter's Mongoose Bite
- Mage's Clearcasting
- Mage's Freeze debuff
- Priest's Surge of Light
- Rogue's Riposte
- Shaman's Elemental Focus
- Warlock's Backlash
- Warlock's Eye of Kilrogg
- Warlock's Nightfall
- Healing Trance / Eye of Gruul, for all healing classes

And here is the list of Glowing Buttons
- Hunter's Counterattack
- Hunter's Mongoose Bite
- Mage's Ice Lance, when the target is Frozen
- Priest's Shadowform, when not assuming the shadowform
- Priest's Smite, during Surge of Light
- Paladin's Exorcism
- Paladin's Hammer of Wrath
- Paladin's Holy Shock
- Rogue's Riposte
- Warlock's Incinerate, during Backlash
- Warlock's Shadow Bolt, during Backlash
- Warlock's Shadow Bolt, during Nightfall
- Warrior's Execute
- Warrior's Overpower
- Warrior's Revenge
- Warrior's Victory Rush

Make sure to report issues and suggest ideas. Thank you :)
## Classes
### Death Knight
New Glowing Buttons
- Blood Tap, with 5+ charges of Blood Charge (Mists of Pandaria)
- Soul Reaper, when the target has low hp (Mists of Pandaria)

There are three Soul Reaper options, one for each spec.

By default, Soul Reaper's execute threshold is 35% of the target's health. This threshold increases to 45% health when the Death Knight equips 4+ pieces of the tier 15 set bonus.
## Bug Fixes
### Glowing Buttons
Glowing Buttons are now functional (The Burning Crusade)
### Druid
In the options panel, the Spell Alert option for Healing Trance (The Burning Crusade, Wrath) now always display the item text next to it.
### Paladin
In the options panel, the Spell Alert option for Healing Trance (The Burning Crusade, Wrath) now always display the item text next to it.
### Priest
In the options panel, the Spell Alert option for Healing Trance (The Burning Crusade, Wrath) now always display the item text next to it.
### Shaman
In the options panel, the Spell Alert option for Healing Trance (The Burning Crusade, Wrath) now always display the item text next to it.
## Known Limitations
Sliders in the options panel have no background ruler (The Burning Crusade)
## Miscellaneous
This release optimizes code size, which have a few positive side effects
- The addon loads slightly faster
- The addon uses slightly less memory
- Packages are slightly smaller to download
- Gains are about 5-15% depending on game flavor and player class

## Contributors
Shout-out to our amazing contributors
- @Amanthuul my man, for adding Death Knights effects to Mists of Pandaria

Thanks!


# Discord

# SpellActivationOverlay 2.7
@here I am pleased to announce the newest release of SpellActivationOverlay, version 2.7

This marks the official release for The Burning Crusade Classic Anniversary :shinto_shrine:  Are you prepared?
## General
### Game Updates
The addon introduces a new flavor: The Burning Crusade.

Here is the full list of supported Spell Alerts so far
- Druid's Nature's Grace
- Hunter's Mongoose Bite
- Mage's Clearcasting
- Mage's Freeze debuff
- Priest's Surge of Light
- Rogue's Riposte
- Shaman's Elemental Focus
- Warlock's Backlash
- Warlock's Eye of Kilrogg
- Warlock's Nightfall
- Healing Trance / Eye of Gruul, for all healing classes

And here is the list of Glowing Buttons
- Hunter's Counterattack
- Hunter's Mongoose Bite
- Mage's Ice Lance, when the target is Frozen
- Priest's Shadowform, when not assuming the shadowform
- Priest's Smite, during Surge of Light
- Paladin's Exorcism
- Paladin's Hammer of Wrath
- Paladin's Holy Shock
- Rogue's Riposte
- Warlock's Incinerate, during Backlash
- Warlock's Shadow Bolt, during Backlash
- Warlock's Shadow Bolt, during Nightfall
- Warrior's Execute
- Warrior's Overpower
- Warrior's Revenge
- Warrior's Victory Rush

Make sure to report issues and suggest ideas. Thank you :)
## Classes
### :dk:  Death Knight
New Glowing Buttons
- Blood Tap, with 5+ charges of Blood Charge (Mists of Pandaria)
- Soul Reaper, when the target has low hp (Mists of Pandaria)

There are three Soul Reaper options, one for each spec.

By default, Soul Reaper's execute threshold is 35% of the target's health. This threshold increases to 45% health when the Death Knight equips 4+ pieces of the tier 15 set bonus.
## Bug Fixes
### Glowing Buttons
Glowing Buttons are now functional (The Burning Crusade)
### :druid:  Druid
In the options panel, the Spell Alert option for Healing Trance (The Burning Crusade, Wrath) now always display the item text next to it.
### :paladin:  Paladin
In the options panel, the Spell Alert option for Healing Trance (The Burning Crusade, Wrath) now always display the item text next to it.
### :priest:  Priest
In the options panel, the Spell Alert option for Healing Trance (The Burning Crusade, Wrath) now always display the item text next to it.
### :shaman:  Shaman
In the options panel, the Spell Alert option for Healing Trance (The Burning Crusade, Wrath) now always display the item text next to it.
## Known Limitations
Sliders in the options panel have no background ruler (The Burning Crusade)
## Miscellaneous
This release optimizes code size, which have a few positive side effects
- The addon loads slightly faster
- The addon uses slightly less memory
- Packages are slightly smaller to download
- Gains are about 5-15% depending on game flavor and player class

## Contributors
Shout-out to our amazing contributors
- @Amanthuul my man, for adding Death Knights effects to Mists of Pandaria

Thanks!

As always, the latest release is available on [CurseForge](https://www.curseforge.com/wow/addons/spellactivationoverlay) / [GitHub](https://github.com/ennvina/spellactivationoverlay/releases/latest) / [Discord](https://discord.com/channels/1013194771969355858/1379111832207228938)